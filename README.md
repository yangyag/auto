...
기본 조회 대상은 현재 환경의 `SYMBOL`과 `STATE_BOT_KEY`다. 현재 USDT 라이브 장부라면 별도 옵션 없이 `KRW-USDT` 기준으로 계산한다.
...
어떤 그리드(슬롯)를 팔아 생긴 실현손익인지 슬롯 단위로 보려면 `scripts/upbit_pnl_by_slot.py` 를 쓴다. 슬롯별 합계와 매도별 상세 2개 섹션을 출력한다.

```bash
.venv/bin/python scripts/upbit_pnl_by_slot.py --period d
```

> **참고가 주의**: 슬롯별 출력의 `그리드매수가`는 **현재 PostgreSQL 그리드 상태 기준 참고가**다. 리센터링 이력이 있으면 과거 매도 당시의 슬롯 가격과 다를 수 있으나, 실현손익 숫자 자체는 실제 체결 FIFO 매칭 기반이라 정확하며 이 참고가에 영향받지 않는다. 같은 슬롯·매도별 손익은 모바일 API `GET /v1/pnl/by-slot` 또는 `/ops` "슬롯별 손익" 버튼으로도 확인할 수 있다.
...
| | `routers/` | health, grid, orders, pnl, monitor, runtime, commands, ops, auth 라우터. pnl 라우터는 `GET /v1/pnl/realized`(기간별 실현손익 합계)와 `GET /v1/pnl/by-slot`(슬롯별 실현손익 + `detail=true` 시 매도별 상세)을 제공 |
| | `schemas/` | API 요청/응답 Pydantic 스키마 (예: `PnlBySlotResponse`/`SlotPnlBucket`/`SellLine`) |
| | `services/` | grid, pnl, monitor, command, runtime 서비스 계층. pnl 서비스는 `calculate_realized_pnl`·`calculate_pnl_by_slot` 로 슬롯 1:1 FIFO 매칭 결과를 응답으로 가공 |
