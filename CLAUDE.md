...
## 현재 로직 핵심
- 첫 가격 스냅샷에서는 빈 슬롯 매수 주문을 만들지 않는다.
- 빈 슬롯은 `previous_price > buy_price >= current_price` 이면 지정가 매수 후보가 된다.
- 빈 슬롯은 하락 구간에서 여러 `buy_price`를 한 poll 안에 함께 통과하면 그 슬롯들이 모두 지정가 매수 후보가 된다.
- 빈 슬롯은 상승 구간에서도 `previous_price < buy_price <= current_price` 인 empty 슬롯이 한 poll 동안 정확히 1개일 때에 한해 그 슬롯만 시장가 예산매수 후보가 된다.
- 보유 슬롯은 현재가가 `sell_price` 이상이면 매도 후보가 된다.
- 같은 루프에 매도와 매수가 함께 생겨도 `main.py`의 매수 주문 판단은 현재 주문 가능 KRW 기준으로 독립 처리한다.
- 하락 교차 매수와 매도는 지정가 주문이다.
- 급등으로 한 poll 안에 `buy_price` 여러 개를 위로 지나간 빈 슬롯은 그 상승 구간 전체를 스킵한다.
- 상승 구간 단일 슬롯 매수만 업비트 `ord_type=price` 시장가 예산매수를 사용한다.
- 주문 상태가 `done`으로 재확인될 때만 해당 슬롯 `held_qty`가 채워져 holding 으로 바뀐다.
