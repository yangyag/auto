...
def _format_row_line(index: int, buy_price, held_qty, sell_price, planned_qty) -> str:
...
def _print_snapshot(snapshot) -> None:
    ...
    print("slot | buy | held | sell | planned | status")
    for row in snapshot.rows:
        print(_format_row_line(row.index, row.buy_price, row.held_qty, row.sell_price, row.planned_qty))
