#!/usr/bin/env python3
"""라이브 그리드의 총예산을 갱신해 planned_qty 만 재계산한다.

DB 의 ladder 와 보유 수량은 그대로 두고, 슬롯별 planned_qty 를 다시 계산한다.
"""
import argparse
import sys
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="라이브 그리드 예산 조정 스크립트")
    parser.add_argument(
        "--target-budget",
        type=Decimal,
        required=True,
...
def build_updated_rows(
    ...
    """현재 DB ladder 기준으로 새 planned_qty 를 계산.

    Returns: (updated_rows, new_planned_buy_budget).
    """
...
def main(argv: list[str] | None = None) -> int:
    ...
    if target_budget <= DECIMAL_ZERO:
        print("에러: --target-budget 은 0보다 커야 합니다.")
        return 1
    ...
    try:
        ...
        try:
            updated_rows, new_planned_buy_budget = build_updated_rows(
                snapshot, target_budget,
            )
        except ValueError as exc:
            print(exc)
            return 1
        ...
        print("-" * 60)
        print(f" 0. 목표 총 예산: {format_decimal(target_budget)} KRW")
        ...
        if target_budget < current_inventory_cost:
            print(" [경고] 목표 총 예산이 현재 보유 중인 인벤토리 가치보다 작습니다!")
