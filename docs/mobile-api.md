...
[run.sh](../run.sh)는 트레이딩 봇만 백그라운드로 실행합니다. 모바일 앱 연동용 API 서버는 `auto-api.service`, 앱의 제어 요청을 실행하는 작업 큐 백그라운드 워커는 `auto-command-worker.service` 서비스로 각각 분리하여 관리됩니다.
...
| [004_mobile_api.sql](../db/migrations/004_mobile_api.sql) | 모바일 API 하트비트, 명령 저장용 큐, Refresh Token 관리용 마이그레이션 SQL |
| [auto-api.service](../deploy/systemd/user/auto-api.service) | FastAPI 서버 구동용 User Systemd 유닛 서비스 설정 파일 |
| [auto-command-worker.service](../deploy/systemd/user/auto-command-worker.service) | 명령 워커 구동용 User Systemd 유닛 서비스 설정 파일 |
...
| :--- | :--- | :--- | :--- |
| **트레이딩 봇** | [run.sh](../run.sh) | 실시간 시세를 모니터링하여 그리드 주문 실행 | 자동매매 전면 중단 |
...
만료된 Access Token을 재발급받습니다. 이때 Refresh Token도 함께 회전(rotation)되어, 응답으로 새 `refresh_token`이 발급되고 요청에 사용한 기존 Refresh Token은 즉시 무효화됩니다. 따라서 클라이언트는 응답으로 받은 새 `refresh_token`을 저장해 이후 갱신에 사용해야 합니다.
...
- **Slot 응답 데이터 필드 정보**:
  ...
  | `planned_qty` / `planned_buy_krw` | number | 매수 대기 상태의 수량(기초자산, cfg.SYMBOL 기준 · 예: BTC/USDT) 및 투입 목표 예산(KRW) |
  | `held_qty` / `inventory_cost_krw` | number | 이미 체결되어 보유 중인 재고 수량(기초자산, cfg.SYMBOL 기준 · 예: BTC/USDT) 및 획득 원가(KRW) |
...
- **Query Parameters**:
  | 파라미터 | 타입 | 기본값 | 용도 |
  | :--- | :---: | :---: | :--- |
  | `limit` | int | `50` | 조회할 최근 데이터의 개수 제한 (허용 범위 `1`~`200`) |
...
### 2-9. 봇 설정값 조회 (`GET /v1/config`)
현재 가동 기준이 되는 봇의 핵심 설정값(거래 마켓, 예산, TP 모델, 가드 활성화 여부 등)을 반환합니다.

- **응답 데이터 필드 정보**:
  | 필드명 | 타입 | 상세 설명 |
  | :--- | :---: | :--- |
  | `symbol` | string | 거래 대상 업비트 마켓 코드 (`cfg.SYMBOL`) |
  | `state_bot_key` | string | 상태 저장에 사용하는 봇 식별 고유 키 |
  | `pg_schema` | string | PostgreSQL 스키마명 (`PGSCHEMA`) |
  | `grid_total_budget_krw` | number | 그리드 총예산 (KRW) |
  | `grid_tp_model` | string | 익절 모델 식별자 (예: `k`) |
  | `grid_tp_k_base` | number | TP K 모델 기준 계수 |
  | `grid_tp_k_floor` | number | TP K 모델 하한 계수 |
  | `upward_buy_enabled` | bool | 상승 추격 매수 활성화 여부 |
  | `breakout_guard_enabled` | bool | 브레이크아웃 가드 활성화 여부 |
  | `stop_loss_mode` | string | 손절 판정 모드 (예: `band_multiple`) |
  | `price_poll_interval` | number | 시세 폴링 주기 (초 단위) |
...
[stop.sh](../stop.sh)를 백그라운드 호출하여 봇을 안전하게 정지시킵니다.
```bash
...
[run.sh](../run.sh)를 실행하여 봇을 백그라운드로 기동합니다.
```bash
...
- 명령 실행기 워커 구동 여부 및 PostgreSQL 명령어 큐 적재 데이터 확인:
  ...
  docker exec auto-postgres psql -U "$PGUSER" -d "$PGDATABASE" \
