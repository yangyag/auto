...
> - **`band_multiple` (기본·권장) 모드**: L0/L1/L2 임계값이 모두 **동일한 단일 밴드 값**으로 산출됩니다(임계값 = $MinBuyPrice \times (1 - k \times band\_ratio)$, level 인자 미사용). 따라서 아래 **L0(신규 매수 차단)·L1(50% 부분청산) 완충 단계가 발동하지 않고**, 시세가 밴드를 하회하면 완충 단계를 건너뛰고 곧바로 **L2(전량 시장가 청산) 단계로 직행**합니다(연속 종가 컨펌은 L2 기준 2회, 대기 시간 30분이 그대로 적용됨).
...
- **봇 거동**: 트리거 확정 시 **신규 매수를 영구 차단**하며, 대기 시간(기본 1시간) 동안 추세를 관측한 뒤 보유 기초자산(cfg.SYMBOL)의 **50%를 지정가로 매도** 처리합니다.
