# Open Sell Order Monitor — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a read-only script that shows each open sell order's real buy cost from bot slots, comparing against current market price

**Architecture:** Single new file `scripts/upbit_open_sell_monitor.py` that reuses `upbit_realized_pnl.run_fifo`/`extract_slot_index` and follows the same API-calling patterns as `upbit_actual_assets.py`. No existing files are modified.

**Tech Stack:** Python stdlib + requests + jwt + project modules (upbit_realized_pnl, app.config.settings)
...
|---|---|---|
| `scripts/upbit_open_sell_monitor.py` | CREATE | CLI entrypoint, API calls, matching logic, output formatting |

---

### Task 1: Create `scripts/upbit_open_sell_monitor.py`

**Files:**
- Create: `scripts/upbit_open_sell_monitor.py`
...
from scripts.upbit_actual_assets import get_with_retry
...
def fetch_open_orders(api_key: str, api_secret: str, market: str) -> list[dict]:
    """Fetch open sell orders from /v1/orders/open with wait+watch states."""
...
def fetch_remaining_buy_queues(
    ...
    from scripts.upbit_actual_assets import _prepare_valid_orders
...
def print_open_sell_summary(
    ...
    header = f"{'slot':>5}  {'qty(BTC)':>12}  {'매수원가':>14}  {'매도지정가':>14}  {'현재가':>14}  {'미실현손익':>14}  도달까지"
    print(header)
    print("-" * len(header))
    ...
    for row in rows:
        ...
        print(f"{slot_str:>5}  {_fmt_btc(row.qty):>12}  {buy_str:>14}  {sell_str:>14}  {cur_str:>14}  {pnl_str:>14}  {gap_str}")
    ...
    print(f"합계: {len(rows)}개 중 {profit_count}개 수익권, {loss_count}개 손실권 | 총 미실현 손익: {total_pnl_str} KRW")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"[진단] open_orders={len(open_orders)}  matched={matched}  unmatched={unmatched}  lookback_days={args.lookback_days}")

    if unmatched > 0:
        print("일부 매도 주문의 매수원가를 찾지 못했습니다. 수동 주문이거나 --lookback-days를 늘려 재확인하세요.")
...
```bash
chmod +x /home/yangyag/auto/scripts/upbit_open_sell_monitor.py
```

- [ ] **Step 3: Run dry syntax check**

```bash
cd /home/yangyag/auto && .venv/bin/python -c "import ast; ast.parse(open('scripts/upbit_open_sell_monitor.py').read()); print('OK')"
```
...
```bash
cd /home/yangyag/auto && .venv/bin/python -c "from scripts.upbit_open_sell_monitor import fetch_open_orders, build_open_sell_rows, OpenSellRow; print('imports OK')"
```
...
```bash
git add scripts/upbit_open_sell_monitor.py
git commit -m "feat: add open sell order monitor script"
```
