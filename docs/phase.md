...
상태: 미진행 (의도적 보류, 현재 운영 범위 아님)

**현재 운영 기준**: 현재 운영은 Phase 7까지 완료된 REST polling/reconciliation 경로를 기준으로 한다.

**착수 조건**: Phase 8은 별도 설계 승인과 시뮬레이션 검증 전에는 진행하지 않는다.
...
- `WebSocket 보류`: 현재 운영은 Phase 7의 REST polling/reconciliation만으로 가능하지만, `myOrder`/`myAsset` 실시간 이벤트가 없어서 체결/취소 반영이 poll 주기만큼 늦고 순간 상태 드리프트를 완전히 없애지는 못한다
- `상태머신 보류`: 지금도 `uuid`/`identifier` 저장과 재조회로 주문 추적은 가능하지만, `trade`/partial fill/`prevented`/재시작 직후 복구를 단일 상태머신처럼 엄밀하게 다루지 못해 예외 상황에서는 보정 로직과 운영자 확인 의존도가 남는다
- `cancel_and_new 보류`: 현재 그리드는 고정 가격대 지정가 운영이라 당장 없어도 돌릴 수 있지만, 빠른 리프라이싱/재배치/잔량 재주문을 원자적으로 처리하지 못하므로 급변장에서는 stale order 와 취소 후 재주문 사이의 공백 리스크를 감수해야 한다
...
- Upbit 엔진 개편은 마지막까지 미루고, 현재 운영은 Phase 7까지만 완료된 상태를 기준으로 본다
